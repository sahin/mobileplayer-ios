...
class URLHelper: MobilePlayerViewController {
  ...
  static func checkURL(url: NSURL, urlType type: URLType, completion: ((
    check: Bool,
    error: NSError?) -> Void)) {
...
extension NSURL {
  ...
  class func
    validateUrl(urlString: String?, completion:(
    success: Bool,
    urlString: String? ,
    error: NSString) -> Void) {
      ...
      if (urlString == nil || urlString == "") {
        completion(
          success: false,
          urlString: nil,
          error: "Url String was empty")
        return
      }
      let prefixes = ["http://www.", "https://www.", "www."]
      for prefix in prefixes {
        if ((prefix.rangeOfString(
          urlString!,
          ...
          locale: nil)) != nil) {
            completion(
              success: false,
              urlString: nil,
              error: "Url String was prefix only")
            return
        }
      }
      ...
      if let test = range {
        completion(
          success: false,
          urlString: nil,
          error: "Url String cannot contain whitespaces")
        return
      }
          ...
          completionHandler:{ (
            ...
            if (error != nil){
              completion(
                success: false,
                urlString: url,
                error: "The url: \(url) received no response")
              return
            }
            if let urlResponse = response as? NSHTTPURLResponse {
              if ((urlResponse.statusCode >= 200 && urlResponse.statusCode < 400) ||
                urlResponse.statusCode == 405) {
                  completion(
                    success: true,
                    urlString: url,
                    error: "The url: \(url) is valid!")
                  return
              }else{
                completion(
                  success: false,
                  urlString: url,
                  error: "The url: \(url) received a \(urlResponse.statusCode) response")
                return
              }
            }
        })
