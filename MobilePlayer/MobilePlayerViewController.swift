...
open class MobilePlayerViewController: UIViewController {
    ...
    public init(contentURL: URL,
        ...
        self.moviePlayer = AVPlayer(url: contentURL)

        super.init(nibName: "MobilePlayerViewController", bundle: nil)
    ...
    private func initializeNotificationObservers() {
        moviePlayer.addObserver(self, forKeyPath: "status", options: .initial, context: nil)
        ...
        (getViewForElementWithIdentifier("close") as? Button)?.addCallback(
            callback: { [weak self] in
                ...
                if let navigationController = slf.navigationController {
                    ...
                    self?.moviePlayer.seek(to: CMTime(seconds: 0, preferredTimescale: 1))
    ...
    public func stop() {
        moviePlayer.seek(to: CMTime(seconds: 0, preferredTimescale: 1))
        moviePlayer.pause()
    }
    ...
    private func parseContentURLIfNeeded() {
        ...
        guard let youtubeID = YoutubeParser.youtubeIDFromURL(url: url) else { return }
        YoutubeParser.h264videosWithYoutubeID(youtubeID) { videoInfo, error in
            ...
            if let urlString = videoInfo.videoURL, let videoUrl = URL(string: urlString) {
                self.moviePlayer.replaceCurrentItem(with: AVPlayerItem(url: videoUrl))
            }
        }
    }
    ...
    private func updatePlaybackInterface() {
        guard let currentItem = moviePlayer.currentItem else { return }

        let currentTime = currentItem.currentTime().seconds
        let duration = currentItem.duration.seconds
        let availableDuration = currentItem.loadedDuration()

        if let playbackSlider = getViewForElementWithIdentifier("playback") as? Slider {
            playbackSlider.maximumValue = Float(duration)
            if !seeking {
                playbackSlider.setValue(value: Float(currentTime), animatedForDuration: MobilePlayerViewController.playbackInterfaceUpdateInterval)
            }
            playbackSlider.setAvailableValue(
                availableValue: Float(availableDuration),
        ...
        if let currentTimeLabel = getViewForElementWithIdentifier("currentTime") as? Label {
            currentTimeLabel.text = textForPlaybackTime(time: TimeInterval(currentTime))
        ...
        if let remainingTimeLabel = getViewForElementWithIdentifier("remainingTime") as? Label {
            ...
            remainingTimeLabel.text = "-\(textForPlaybackTime(time: TimeInterval(remainingTime)))"
            remainingTimeLabel.superview?.setNeedsLayout()
        }
        if let durationLabel = getViewForElementWithIdentifier("duration") as? Label {
            durationLabel.text = textForPlaybackTime(time: TimeInterval(duration))
    ...
    private func handleMoviePlayerPlaybackStateDidChangeNotification() {
        state = StateHelper.calculateStateUsing(previousState: previousState, andPlaybackState: moviePlayer.status)
    ...
    private func updateShownTimedOverlays() {
        ...
        DispatchQueue.global().async {
            for timedOverlayInfo in self.timedOverlays {
                if timedOverlayInfo.startTime <= currentTime.seconds && currentTime.seconds <= timedOverlayInfo.startTime + timedOverlayInfo.duration {
