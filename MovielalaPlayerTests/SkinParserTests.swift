...
class SkinParserTests: XCTestCase {
    ...
    if let config: MovielalaPlayerConfig =
      SkinParser.parseConfigFromURL(fileURL) {
        XCTAssertEqual(
          ...
          UIColor(hexString: "#ffff00"), "timeText tint color not equal")
        XCTAssertEqual(
  ...
  func testCOntrolBarConfigButtons() {
    if let config: MovielalaPlayerConfig =
      SkinParser.parseConfigFromURL(fileURL) {
        XCTAssertEqual(
        ...
        XCTAssertEqual(
          ...
          UIColor(hexString: "#ffff00"), "pauseButton tint color not equal"
        )
    }
  }

  func testControlBarConfigImages() {
    if let config: MovielalaPlayerConfig =
      SkinParser.parseConfigFromURL(fileURL) {
        XCTAssertEqual(
  ...
  func testTitleConfig() {
    if let config: MovielalaPlayerConfig =
      SkinParser.parseConfigFromURL(fileURL) {
        XCTAssertEqual(config.titleConfig.backgroundColor,
          UIColor(hexString:"#ff00ff"),
          "Title background color not equal"
        )
        XCTAssertEqual(config.titleConfig.textColor,
          UIColor(hexString:"#ffff00"),
  ...
  func testCloseButton() {
    if let config: MovielalaPlayerConfig =
      SkinParser.parseConfigFromURL(fileURL) {
        XCTAssertEqual(config.closeConfig.tintColor,
          UIColor(hexString:"#ffff00"),
          "Close Button tint color not equal"
        )
        XCTAssertEqual(config.closeConfig.backgroundColor,
          UIColor(hexString:"#ff00ff"),
